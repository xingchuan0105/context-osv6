#!/usr/bin/env bash
# Re-run full-149 non-PASS questions with OpenCode Go as agent + judge.
# Merges into a baseline merged.json (or phase1 summary) when provided.
#
# Usage:
#   BASELINE_MERGED=.../full149_..._merged.json \
#   E2E_QUESTIONS=17,20,... \
#   bash scripts/run-full149-nonpass-opencode.sh
set -uo pipefail
...
RUN_TAG="full149_nonpass_opencode_${STAMP}"
...
export AGENT_LLM_MODEL="${AGENT_LLM_MODEL_OVERRIDE:-${QWEN_CHAT_MODEL:-deepseek-v4-flash}}"
...
# Do not re-ingest unless explicitly requested
...
if baseline_path:
    ...
    if bp.is_file():
        ...
        # prefer explicit per_question_merged; else reconstruct
        ...
        for k, v in pq.items():
            try:
                ...
                pass
        # if baseline only has summary, keep only rerun overlay
        if not merged and baseline_meta.get("phase1_run_dir"):
            p1 = Path(baseline_meta["phase1_run_dir"])
            merged = load_labels(p1)

# overlay rerun
for qid, lab in rerun.items():
    if qid in rerun_ids or True:
        merged[qid] = lab

counts = Counter(merged.values())
pass_n = counts.get("PASS", 0)
total = len(merged) if merged else len(rerun)
if not merged:
    merged = dict(rerun)
    counts = Counter(merged.values())
    pass_n = counts.get("PASS", 0)

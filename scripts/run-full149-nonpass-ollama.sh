#!/usr/bin/env bash
# Re-run full-149 non-PASS questions with Ollama Cloud as agent, OpenCode Go as judge.
# Merges into BASELINE_MERGED when provided.
#
# Usage:
#   BASELINE_MERGED=.../merged.json E2E_QUESTIONS=17,20,... E2E_CONCURRENCY=8 \
#     bash scripts/run-full149-nonpass-ollama.sh
set -uo pipefail
...
RUN_TAG="full149_nonpass_ollama_${STAMP}"
...
if [ -z "${E2E_QUESTIONS:-}" ]; then
  echo "[run] ERROR: E2E_QUESTIONS required" | tee -a "$LOG"
  exit 2
fi
...
if baseline_path:
    ...
    if bp.is_file():
        ...
        if not merged and baseline_meta.get("phase1_run_dir"):
            merged = load_labels(Path(baseline_meta["phase1_run_dir"]))

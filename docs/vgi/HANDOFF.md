...
**M2 答题级对照**（同 20 题、同 Eval v2 裁判）：vector **25%** → hybrid **20%** → hybrid+rr200 **17.5%**（@5 轮预算）；同一 hybrid+rr200 臂**放宽到 20 轮/64 调用/1800s 后 35%**（7/20：435/411/503/89/684/380/20，REFUSAL_WRONG 清零，token 434k→3.87M）。结论：5 轮预算才是真实瓶颈；放开后检索增益兑现（411/89 首答对）。未跑 vector@20 轮对照臂，臂间差异归因到检索的置信度有限。

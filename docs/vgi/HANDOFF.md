...
agent 级脚本 `scripts/challenge20-answer-eval.py`：环境变量切臂 `ANSWER_EVAL_{MODE,RERANK,THINKING,MAX_ROUNDS,MAX_TOOLS,DEADLINE_S,CLOSEOUT_MARGIN_S,OUT}`。第 5 个工具 `vgi_ledger`：查证台账（约束清单 open/verified/refuted + evidence 指针 + hypothesis），不占工具预算、每轮回显到上下文尾部；剩余 < CLOSEOUT_MARGIN_S（默认 120，上限 DEADLINE_S/3）提前触发 closeout（摘工具、关 thinking），deadline 空答案回退到台账 hypothesis——为 thinking 三病理所加。`ANSWER_EVAL_SAC=1` 切 SAC 臂：单工具 `vgi_code` 在子进程 Python 运行时执行模型代码（async 函数体），预置异步 SDK `search/read/rg/related` + 同步 `ledger`，`asyncio.gather` 真并发、中间结果留在代码变量不进上下文；SDK 事件回流保裁判 context 口径一致，预算按检索类调用计（`ANSWER_EVAL_CODE_{TIMEOUT_S,MAX_CALLS}` 默认 240s/12）。语料规模观测消息（`corpus_brief()`，从 corpus.sqlite 现算 n_docs/中位长度/窗口覆盖率）作为第二条 system 消息注入，双臂共用。arm 标签已动态化。思维链落盘 `reasoning-{qid}.txt`（需在进程启动前改代码——Python 不热加载）。

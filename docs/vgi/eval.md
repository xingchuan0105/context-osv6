...
命令：`vgi eval --questions <json> [--mode vector|lexical|hybrid]` 对每题跑对应检索路（M1 只有 vector；M2 起三 mode 都可评），写 `.vgi/jobs/eval-<profile>.json`：`recall` 在 k∈{5,100,1000} 的均值，加 `per_question`，protocol 记录 mode。均值 = `(1/n) Σ recall_at_k`。

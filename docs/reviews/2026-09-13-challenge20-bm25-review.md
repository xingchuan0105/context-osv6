...
> 先澄清"检索放在 agent 循环里"的两层含义，避免误读：
> - **BM25 标定（本评审对象）完全无循环**：一个问题 → 一次 BM25 → top-50 计分，没有 agent、没有改写。
> - **run-9 树实验有 agent 循环**（5 轮工具 + 强制收尾），但循环里只有字面 `grep` / `tree_overview` / `read`，**没有排序检索器**：模型拿不到"相关文档列表"供提炼实体再查，只能盲猜短语去全库 grep；且 5 轮预算里 grep 单次 26–45 秒、26–52% 超时零返回。
> - 官方 agent 的循环首步就是排序检索器（BM25/稠密返回 top-5 文档），榜单样例平均每题检索调用约 **12.6 次**。所以"把检索放进 agent 循环"目前**还没有做到**——缺的不是循环，是循环里的排序反馈面。

...
按序叠加，每项一个可单测的 reducer/模型缝 + 一个浏览器旅程：Markdown（`pulldown-cmark` 事件过滤，恶意 fixture）→ 引用 marker 与来源卡 → 进度 / 推理区终态折叠 → Session files（signed upload）→ scope bar（Session / Workspace / Web 增量叠加）→ `model_role` 与 quick-chat BYOK 读取 → feedback → `/dashboard/:workspace_id` 内复用同一 ChatCanvas。

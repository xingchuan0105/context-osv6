...
- 用户随后完成输入法检查并确认“输入法没问题”，中文 IME 候选/组字验收通过。客户端重启恢复与服务端模型取消尚未验证。Markdown 原文符号直接显示及历史均为“未命名对话”的问题仍待修复。
- D1 Windows 基础聊天验收通过，完整视觉与客户端对等未验收；D2–D6 未据此推进，Tauri 保留。

## 2026-09-10 阅读与标题修正

后续已接入现有组件库的 Markdown 渲染，使用首条问题命名无标题会话并经现有接口保存，补齐侧栏选中状态与长标题提示。Windows UI 检查、构建和 34 项自动测试通过，新窗口已启动，视觉与新版交互等待用户操作确认。实现及证据见 [Markdown 与会话标题](../desktop/2026-09-10-gpui-markdown-titles.md)。

用户随后提供截图并反馈“应该都正常了”，主体验确认。截图和历史原文复核仍有中文括号旁加粗标记、有序列表起始编号两处排版余项，记录待修，不据此宣布全部视觉问题清零。

后续余项代码已修正：通过现有 TextView 块扩展接入 Comrak 的中文加粗规则，保留列表原始起始编号及嵌套结构，不改写历史原文。6 项新增定向测试、共 40 项共享套件测试和 Windows UI 编译检查、构建通过；用户关闭旧窗口后，新版验收程序已打开。用户随后回复“已检查，全部正常”，确认中文加粗、列表起始编号及含列表正文的选择/复制正常，本批原生复验通过。范围与证据继续记录于 [Markdown 与会话标题](../desktop/2026-09-10-gpui-markdown-titles.md)，未推进 D2–D6。

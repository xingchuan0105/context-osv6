...
后续余项代码已修正：通过现有 TextView 块扩展接入 Comrak 的中文加粗规则，保留列表原始起始编号及嵌套结构，不改写历史原文。6 项新增定向测试、共 40 项共享套件测试和 Windows UI 编译检查、构建通过；用户关闭旧窗口后，新版验收程序已打开，原生复验待用户操作。范围与证据继续记录于 [Markdown 与会话标题](../desktop/2026-09-10-gpui-markdown-titles.md)，未推进 D2–D6。

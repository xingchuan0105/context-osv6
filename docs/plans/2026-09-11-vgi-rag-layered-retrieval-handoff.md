...
**后续实现已落地并通过本地机制验证。** 当前公共管线、六模块、配置与 G0–G5 关闭状态见 [模块化实施记录](2026-09-11-vgi-rag-modular-implementation.md)。[审计记录](2026-09-11-vgi-rag-modular-prototype-audit.md) 和下文实验属于修改前的快照；新架构尚未进行真实在线质量与规模性能验收。
...
前一轮混合发现局部改动有 39 项本地检查。后续已用共享 RetrievalPipeline 替代 HardAgent 的 seed_pool/expand_seed，接入各模块并将默认显示上限接到 30。以下数字对应历史配置，不能作为这些新模块的成绩；第 5 节设计示意已由实施记录链接的可执行合同取代。

## 1. 先读结论

1. **历史六组开发集已跑通**（A–E 本地 Agent，F 走产线 SAC；两侧模型统一 `qwen3.8-flash`）。在 248 份语料 × 20 道开发题上，尚未测出图＋树相对强混合检索的比较优势。种子覆盖率高可能压缩扩张收益，但规模比例本身不能证明原因；旧自动图路径未遍历边等实现缺陷也需要分开验证。
2. **不要放弃向量。** @15 证据覆盖：在线 dense 63.6% vs 本地 TF-IDF 46.7% / LSA 40.2%；混合 BM25+在线向量 66.4% vs BM25+本地 56.1%。放弃向量稳定损失约 10 个百分点。
3. **旧实现中，相似图的开发集收益未被证实。** embedding 块 kNN 图相对随机对照 +0.3~−0.7；窗口质心图 −1；当时 `expand_seed` 只调 annotate 找自身窗口，没有沿边自动扩张。相同 embedding 空间不等于图结构必然无用；新自动遍历与融合已经补齐，质量贡献仍待对照。
...
7. **多跳采用查询时抽取（路线 B）。** 历史估算入库全量约 857M tokens、查询抽取每千次约 44M，直接相除的打平点约 19,500 次查询；原 13,000 次缺少对应假设。统计抽词和专名跳转的开发集探针不等同于真实 LLM 抽取质量。新实现默认每次最多抽 4 份种子文档，成本需重新测量。

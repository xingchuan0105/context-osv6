...
以上保留首次交付及当时的待验状态。2026-09-10 完整受管进程自动验收已通过：共享/无头 60 项、真实数据栈 4 步、迁移/API/worker/本机会话 8 步通过。S2/S6 的失败恢复、启动超时中退出和进程清理自动门关闭，D2 Windows 可以进入 D3；不要求逐项人工确认。当前范围、构建耗时、产物哈希与结果路径以 [受管进程验收记录](2026-09-10-gpui-managed-acceptance.md) 为准。现有窗口未被本批更新，Tauri 保留；系统文件管理器、GPU 像素与其他平台仍未验。

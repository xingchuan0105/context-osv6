...
| # | 能力 | 状态 |
|---|---|---|
| D2 | 栈状态、启动迁移、进程/日志、退出收摊 UI（S0–S6，不依赖预装 Docker） | 已实现服务面板与生命周期接线；45 项单测/夹具、1 项真实 API 只读探测及 Windows 构建通过。面板用户验收、受管环境 S2/S6 真机门待验；[本批证据](2026-09-10-gpui-services.md) |

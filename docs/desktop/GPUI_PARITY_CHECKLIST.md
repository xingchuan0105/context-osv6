...
2026-09-10：D1 Windows 基础聊天验收通过。随后已实现 Markdown 渲染及会话标题保存，Windows 构建和 34 项自动测试通过；用户确认新版主体验，截图验证标题、选中状态和大部分排版生效。中文括号旁加粗标记及有序列表起始编号仍有余项（[本批证据](2026-09-10-gpui-markdown-titles.md)）。完整视觉与客户端对等未验收；D2–D6 状态不变，Tauri 保留。

...
| # | 能力 | 状态 | 证据 |
|---|---|---|---|
| D1.1 | gpui-kit 依赖锁定 rev / 窗口 / composer / 中文 IME（真实输入无重复字） | 实现；Windows 构建通过，原生待验 | [2026-09-09 D1](../plans/2026-09-09-gpui-chat-task.md)：自动启动被审批拦截 |
| D1.2 | 共享 reducer 流式显示、local session 真 API、取消、历史（127.0.0.1:18080） | 接线实现；真实链路待验 | [共享套件验收](2026-09-09-gpui-tauri-suite-acceptance.md) 31 项通过，含模拟本地登录/历史及 TCP 取消；原生 UI / 真实模型待验 |

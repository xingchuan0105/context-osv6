...
2026-09-10：D1 Windows 基础聊天验收通过。随后已实现 Markdown 渲染及会话标题保存，用户确认新版主体验。截图发现的中文括号旁加粗及列表起始编号余项已补充解析/渲染修正，40 项自动测试和 Windows UI 编译检查、构建通过；用户关闭旧窗口后，新版已打开，新版原生复验待用户操作（[本批证据](2026-09-10-gpui-markdown-titles.md)）。完整视觉与客户端对等未验收；D2–D6 状态不变，Tauri 保留。

...
用户完成中文输入法检查后确认“输入法没问题”，原生中文 IME 候选/组字检查由用户确认通过。至此，D1 Windows 基础聊天验收通过：本机连接、真实回答、流式显示、停止保留正文、停止后续聊、会话切换历史恢复及中文输入均已有用户窗口确认。此结论不代表完整客户端或视觉验收通过；Markdown 渲染和会话名称仍待修复，客户端重启恢复和服务端模型取消仍未验证，D2–D6 状态不变。

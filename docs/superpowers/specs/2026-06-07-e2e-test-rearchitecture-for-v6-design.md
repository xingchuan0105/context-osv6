...
└── specs/
    ...
    │   ├── workspace-chat.spec.ts      # 降级为 smoke，精简断言 → 保留在 journey 做完整闭环
    │   ├── workspace-upload-rag.spec.ts
    │   ├── workspace-share.spec.ts
    │   ├── chat-session.spec.ts        # ← 从 avrag-rs 迁移
    │   ├── notebook-crud.spec.ts       # ← 从 avrag-rs 迁移（API → UI 改造）
    │   ├── session-history.spec.ts     # ← 从 avrag-rs 迁移
    │   ├── tenant-isolation.spec.ts    # ← 从 avrag-rs 迁移
...
| `pom/workspace-page.ts` | 重构吸收 `ChatPage.ts` | 统一 POM 接口 |

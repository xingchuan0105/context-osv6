...
**2026-09-10 GPUI 本机服务入口**：聊天上下文顶栏的“本机服务”打开原生服务面板，可返回原聊天并保留草稿与历史。此入口属于 J7 的本机运行管理，显示实际连接与生命周期状态，不占用工作区主导航位置，也不创建第二套业务设置或云端登录入口。

**2026-09-10 GPUI D3 知识路径**：原生左栏按「个人聊天」「工作区」排列，工作区入口先到总览；进入对象后展示该工作区会话和「工作区新对话」。顶栏「资料」「笔记」直接打开对应面板，「添加资料」直接选择文件，取消不展开。资料按上传/处理中/可检索/失败显示，问答限定当前工作区，个人聊天不携带工作区或 RAG 能力。资料面板收起保留任务和笔记草稿；离开未保存笔记时确认。引用在回答旁有可打开的来源入口，使用现有鉴权接口读取原文。原生页面复用已有 Workspace API，不新增知识容器或业务接口。
...
**2026-09-08 Rust 修正实现边界（本地构建与回归已通过，未部署）**：个人附件复用既有 PDF / Office（含 Excel、PPT）/ 文本 / 图片 OCR 解析器，走独立 parse-only 请求；不创建原件入库副本、DocumentBinding 或索引任务。前端在发送前持有提取文字；发送后仅保留当前轮重试所需内容，下一新轮替换；服务端会话历史只保留文件名。刷新后重用文件需要重新添加。解析器临时文件按既有解析器生命周期清理；外部 OCR 的数据处理遵循该服务配置，不能将本地无入库等同于外部服务零留存。办公文档以提取文字为上下文，不承诺内嵌图像的完整视觉理解。真实 Office 解析已测试，外部图片 OCR、真实模型和线上验证码尚未验收。

初始容量上限：每轮 5 个非空文件、每文件 20 MiB；问题与提取文字的请求上下文上限 64 KiB，服务端同时校验序列化上下文大小。超出上限明确报错，不截断、不转检索。工作区资料勾选表示本次问答限定范围；未勾选表示整个当前工作区的已就绪资料，非就绪资料不可勾选。工作区使用自身会话列表，复用聊天画布不重复展示个人会话侧栏。分享问答使用独立会话状态、分享 token 和分享所属工作区，不导航到访客的个人会话。
...
| 升级会员 | `/pricing`（档位区） | 顶栏分享组「升级」、升级弹窗「详情」、paywall、分享转化「升级」；档位区选择真实方案直接购买，与余额充值分别结账 |
...
### 2026-09-10 GPUI 界面收口

GPUI 原生壳沿用下述已确认布局尺寸与功能边界。全局导航为个人聊天、工作区总览，新对话是独立动作；历史仅显示当前个人/工作区作用域。768px 以下导航使用 Kit 原生模态抽屉，768–1199px 默认 60px 短文字导航，1200px 起默认 248px，桌面折叠偏好与明暗外观本机保存。资料 1200px 以下使用原生抽屉，宽屏 336px 并排；资料/笔记直达、添加文件、未保存保护不变。服务状态只有页头一个入口；外观在导航底部。不用假账户、假模型选择或失效工具按钮填充尚未实现的功能。实施与验证范围见 [本批计划](../plans/2026-09-10-gpui-ui-finish.md)。

### 2026-09-08 Rust 布局验收决策

侧栏可辨识性修订：折叠导航仍显示图标及短文字，不以无文字的加号表示聊天目的地；第二个全局入口固定为「工作区」，进入 `/dashboard` 总览，具体工作区不从聊天历史推导成主导航。聊天目的地与「新对话」动作分别命名；工作区内创建会话标为「工作区新对话」。历史会话作为独立分区，工作区总览在该对象的局部导航之前。

用户已验收 W0 样稿；以下规则覆盖下表中 Rust 的旧壳规定，Next 本轮不改。Rust 应用采用 248px/60px 左导航、52px 上下文顶栏；账户位于导航底部，通知只出现一次。个人聊天顶栏不放分享/营销菜单，主阅读区上限 760px，输入区采用文本加单工具栏，个人聊天没有知识库入口。

768px 以下导航为抽屉，768–1199px 默认折叠，1200px 起可展开并保存桌面折叠偏好。工作区资料默认收起，资料/笔记入口直接打开对应分区；顶栏添加资料直接选择文件，选定后展开处理状态，取消选择不展开。收起面板不取消上传或清除笔记草稿；离开有未保存笔记的工作区时确认。现有笔记创建、列表、预览、删除能力保留。

设置/分享采用分类加内容布局，手机采用分类详情；公开、认证、管理、文档按任务适配。此决策按 W1–W5 分批落实，未迁移页不代表已完成；唯一完成路径仍遵循 §4。

W3 实施规则：设置桌面左分类右内容，手机 `/settings` 为分类列表、`?tab=` 为分类详情；`/settings/usage` 归入同一分类导航。未知 tab 显示个人资料，已有 providers/billing 等深链有效。分享中心、访问日志、分析共用对象内分类导航，并保留返回该工作区的入口。公开知识库、作者、邀请采用轻量公共页头，不请求私人会话列表。会员/充值仍只跳转 `/pricing` 与 `#topup`。

W4 实施规则：管理页使用应用壳、局部管理导航和宽数据区，移动端局部导航可横向滚动；帮助首页与写作说明使用应用壳，其余公开说明、集成、认证、激活、配置及付款结果使用轻量公共壳。商业和法律页保留双语公共站点页头；应用中不保留旧 AppTopBar、全局分享菜单或营销页脚。有效路由和对象路径不变。

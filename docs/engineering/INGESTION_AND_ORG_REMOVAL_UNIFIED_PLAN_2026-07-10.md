...
2. **Org**：产品已无 org 语义，但代码/schema/RLS/Admin 仍以 `owner_user_id` 为租户轴 → agent 读库必歧义。按 workspace rename 同纪律 **彻底去掉 org 概念**，租户轴改为 **账号所有者 `user_id`（owner）**，资源范围以 **workspace** 为准。
...
Wave O1 (schema + RLS)         列/表/策略迁移：owner_user_id → owner_user_id（或等价）
...
Wave O5 (admin + docs + rg)    Admin 去 accounts；全库禁词验收
```
...
- 查 derived 表：必须设置 **当前租户 GUC**（O 波前为 `app.current_user`；O 波后见 B.3）。  
...
```sql
-- O 波前：派生表 RLS 按 org
SELECT set_config('app.current_user', '<org_uuid>', false);
...
| # | 决策 |
|---|------|
| 1 | **产品与代码公开层不再使用 org / account / UserId 作为业务概念** |
| 2 | **租户隔离轴** = 账号所有者 **`user_id`（owner）**；**协作/资料范围** = **`workspace_id`** |
| 3 | **删除** `accounts` 表与 Admin「组织」面；运营只认 **users** |
| 4 | **删除** `x-owner-user-id`、JWT `owner_user_id` claim、MCP `org.*` 工具名、`/org/*` 路由 |
| 5 | **存储硬切**：列 `owner_user_id` → `owner_user_id`（全库统一命名，见 B.3）；GUC `app.current_user` → `app.current_user`（与计费 RLS 对齐） |
| 6 | **无长期 alias**（不保留 `owner_user_id` JSON 字段双写）；未上线，允许破坏性 migration |
...
**注册**：不再 `INSERT accounts`；`users` 不再 `owner_user_id` FK。  
...
| 「Auth/RLS 全是 org」 | `AuthContext`、PG RLS、Milvus | agent 每个 SQL 都写 owner_user_id |
| 「Admin 管 accounts」 | admin routes/UI | 以为是 B2B 多租户 SaaS |

结果：诊断写错 GUC、新 API 误加 `owner_user_id`、测试夹具伪造 `org-1`、与 workspace 主语义冲突。
...
|----|------------|------|
| `accounts` 表 | **删除** | 无替换表 |
| `owner_user_id` 列（资源表） | **`owner_user_id`** | 资源归属账号；**不是**「操作者」时仍用此列做 RLS |
| `users.owner_user_id` | **删除列** | user 自身即根 |
| `UserId` / `owner_user_id()` | **`UserId` / `user_id()`**（contracts 已有或补齐） | runtime |
| `app.current_user` | **`app.current_user`** | 与 0035 subscriptions RLS 一致 |
| `x-owner-user-id` | **删除**；鉴权只认 JWT / API key /（测试）`x-user-id` | 禁止新 proxy org header |
| JWT claim `owner_user_id` | **删除**；保留 `user_id` + `auth_version` | |
| MCP `account.create_workspace` | **`workspace.create`**（或现有 `workspace.*` 已覆盖则删） | wire 禁 `org.` 前缀 |
| 路由 `/org/api-keys` | **`/account/api-keys` 或 `/me/api-keys`** | 账号级 key（若仍需要） |
| Admin `/admin/accounts` | **删除**；能力并入 `/admin/users` | |
| Object key `{org}/{ws}/{doc}/…` | **`{owner_user_id}/{workspace_id}/{doc}/…`** | 迁移脚本 rewrite 或双读一波内切完 |
| Milvus `owner_user_id` field | **`owner_user_id`**（或复用现有字段 rename） | filter 同步 |

**禁止**用 `tenant_id` 当「改名 org」继续走私；新代码若需要隔离词，只用 **`owner_user_id` / `user_id` / `workspace_id`**。

## B.4 现状库存（执行时再 `rg` 刷新）

量级（2026-07-10 扫描）：**~250+ 文件** 含 `owner_user_id` / `current_org`（除 target/node_modules）。

| 层 | 代表路径 |
|----|----------|
| Contracts | `contracts/src/auth_runtime.rs`（`UserId`）、`workspaces.rs` / `documents.rs` / `admin.rs` |
...
| Worker / ingestion | task payload `owner_user_id`；parse_run insert |
| Milvus | `storage-milvus` `doc_filter` org |
| Object store | 路径前缀 org |
| Frontend | `app/admin/accounts/**`、`lib/admin/client.ts`、generated contracts |
...
| ID | 任务 | 验收 |
|----|------|------|
| **O1.1** | 新 migration：资源表 `owner_user_id` → `owner_user_id`（数据：`owner_user_id = users.id` 从旧 org 映射——取该 org 下主用户/最早 admin/唯一 user） | up/down 可逆或标注 irreversible |
| **O1.2** | `users` 去 `owner_user_id`；`DROP accounts CASCADE` 前拆 FK | 无 accounts |
...
```text
owner_user_id := (SELECT id FROM users WHERE users.owner_user_id = <old_org> ORDER BY created_at ASC LIMIT 1)
```
...
| ID | 任务 | 验收 |
|----|------|------|
| **O2.1** | `AuthContext` 去掉 `UserId`；`new(user_id, subject_kind)` | contracts 测试绿 |
| **O2.2** | JWT 只签 user；删 `x-owner-user-id` 解析 | 登录/刷新 E2E 或 L1 |
...
| ID | 任务 | 验收 |
|----|------|------|
| **O4.1** | contracts DTO 去 `owner_user_id`；重生 TS | `pnpm` typecheck |
...
| ID | 任务 | 验收 |
|----|------|------|
| **O5.1** | 删除 admin accounts 页与 API；users/usage 只按 user | admin 可列用户 |
...
| 点 | 处理 |
|----|------|
| 诊断 SQL 里的 `owner_user_id` 列 | I 波可暂用旧列；O1 后改 `owner_user_id` |
...
1. **禁止** 新代码引入 `org` / `UserId` / `owner_user_id` / `accounts` / `x-owner-user-id` / `app.current_user`。  
...
- [ ] DB 无 `accounts` 表；资源表无 `owner_user_id`  
- [ ] 运行时无 `UserId` / `x-owner-user-id` / JWT org claim  

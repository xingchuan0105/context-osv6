...
| Deploy | Local → VPS via formal scripts | **Only** `scripts/deploy-frontend.sh` / `deploy-backend.sh` / `deploy-public-sites.sh` / `publish-desktop-release.sh` (+ `deploy-status.sh`). Not ad-hoc scp of product code; not “must pass GitHub PR checks first”. See [`LOCAL_VPS_ALIGNMENT_PLAN_2026-07-14.md`](./LOCAL_VPS_ALIGNMENT_PLAN_2026-07-14.md). |

fn notebook_card_sections(
    ...
    let cards = notebooks
        ...
        .map(|notebook| {
            ...
            let notebook_date = format!(
                "{} {}",
                choose(current_locale, "最近更新", "Updated"),
                dashboard_notebook_date_label(current_locale, &notebook.updated_at)
            );
            ...
            view! {
                <A
                    ...
                    attr:class=dashboard_style::card
                >
                    <div class=dashboard_style::card_top>
                        <div class=dashboard_style::menu_anchor>
                            <button
                                type="button"
                                class=dashboard_style::card_menu_button
                                on:click=move |ev| {
                                    ev.prevent_default();
                                    ev.stop_propagation();
                            ...
                            <Show when=move || open_menu_id.get().as_deref() == Some(notebook_id_for_menu_visibility.as_str())>
                                <div class=format!("{} {}", dashboard_style::menu, dashboard_style::card_menu)>
                                    <button
                                        type="button"
                    ...
                    <div class=dashboard_style::card_body>
                        <h3 class=dashboard_style::card_title>
                            {notebook_title}
                        </h3>
                        <p class=dashboard_style::card_description>
                            {notebook_description}
                        </p>
                    </div>

                    <div class=dashboard_style::card_meta_row>
                        <Show when=move || is_favorite>
                            <span class=dashboard_style::card_chip>
                    ...
                    <div class=dashboard_style::card_date>
                        {notebook_date}
                    </div>
                </A>
            }

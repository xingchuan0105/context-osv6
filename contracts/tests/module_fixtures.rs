use contracts::{HealthResponse, WorkspaceListResponse};

// Restored from the pre-C3 `module_fixtures.rs` (P1-1): the C3 billing commit
// deleted this whole file, taking two billing-unrelated fixture round-trip
// tests with it. The billing_plans fixture was correctly retired with the
// billing contract rework; these two are back.

#[test]
fn workspace_list_minimal_fixture_roundtrips() {
    let json = serde_json::json!({
        "workspaces": [{
            "id": "nb-1",
            "owner_user_id": "user-1",
            "owner_id": "user-1",
            "name": "demo",
            "title": "Demo",
            "description": "",
            "created_at": "2026-01-01T00:00:00Z",
            "updated_at": "2026-01-01T00:00:00Z",
            "document_count": 0,
            "status_summary": {},
            "shared": false
        }]
    });

    let parsed: WorkspaceListResponse =
        serde_json::from_value(json.clone()).expect("workspaces list should deserialize");
    assert_eq!(parsed.workspaces.len(), 1);
    assert_eq!(parsed.workspaces[0].name, "demo");

    let serialized = serde_json::to_value(parsed).expect("workspace list should serialize");
    assert_eq!(serialized, json);
}

#[test]
fn admin_health_minimal_fixture_roundtrips() {
    let json = serde_json::json!({
        "status": "ok",
        "service": "avrag-api",
        "version": "0.1.0"
    });

    let parsed: HealthResponse =
        serde_json::from_value(json.clone()).expect("health response should deserialize");
    assert_eq!(parsed.status, "ok");
    assert_eq!(parsed.service, "avrag-api");

    let serialized = serde_json::to_value(parsed).expect("health response should serialize");
    assert_eq!(serialized, json);
}

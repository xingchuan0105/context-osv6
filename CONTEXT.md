...
| **LiteParse Ingestion (P4)** | Current PDF/Office→PDF parse path: hybrid probe (`probe_pdf_hybrid`) → per-page routing (`router/page_routes`) → LiteParse text extraction + Paddle Jobs OCR + VisualRaster fallback. MinerU and shadow/rollout switches removed. See `avrag-rs/docs/liteparse-paddle-ingestion-architecture-2026-06-13.md`. | Ingestion (`crates/ingestion/src/parser/`) |

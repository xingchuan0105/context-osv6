...
fn wait_port(startup: &Deadline, host: &str, port: u16, secs: u64, log: &mut String) -> bool {
...
fn run_capture(deadline: &Deadline, cmd: &mut Command) -> (i32, String, String) {
    apply_windows_no_window(cmd);
    match deadline.output(cmd) {
        Ok(o) => (
...
fn run_status_null(deadline: &Deadline, cmd: &mut Command) -> i32 {
    apply_windows_no_window(cmd);
    cmd.stdin(Stdio::null())
        .stdout(Stdio::null())
        .stderr(Stdio::null());
    match deadline.status(cmd) {
...
fn write_client_env(
    ...
    let pg_port = runtime_ports::postgres();
    let redis_port = runtime_ports::redis();
    let api_port = runtime_ports::api();
...
AVRAG_API_ADDR=127.0.0.1:{api_port}
AVRAG_PUBLIC_BASE_URL=http://127.0.0.1:{api_port}
CORS_ALLOWED_ORIGINS=http://tauri.localhost,https://tauri.localhost,http://127.0.0.1:{api_port},http://localhost:{api_port},http://localhost:3000,http://127.0.0.1:3000
...
fn ensure_postgres(deadline: &Deadline, pg_bin: &Path, pgdata: &Path, run_dir: &Path, log_file: &Path, log: &mut String) -> Result<(), String> {
    let pg_port = runtime_ports::postgres();
    deadline.check()?;
    ...
    if port_open("127.0.0.1", runtime_ports::postgres()) {
    ...
    if pid_marker.is_file() && !port_open("127.0.0.1", runtime_ports::postgres()) {
    ...
    if !pgdata.join("PG_VERSION").is_file() {
        ...
        let (code, out, err) = run_capture(deadline, &mut init);
        if code != 0 {
            append_log(log, out);
            append_log(log, err);
...
host    all             all             ::1/128                 trust\n"
    ...
    if !port_open("127.0.0.1", runtime_ports::postgres()) {
        ...
        start
            ...
            .arg(pgdata)
            .arg("-p")
            .arg(runtime_ports::postgres().to_string())
            .arg("-c")
        ...
        if let Err(error) = fs::write(run_dir.join("postgres-native.pid"), format!("{}\n", child.id())) {
            crate::process_deadline::terminate_and_wait(&mut child)?;
            return Err(format!("record postgres process: {error}"));
        }
        drop(child);
    ...
    if !wait_port(deadline, "127.0.0.1", runtime_ports::postgres(), 45, log) {
    ...
    for _ in 0..60 {
        ...
        for user in [PG_ADMIN_USER, PG_USER] {
            let (code, _, _) = run_capture(deadline,
                Command::new(&psql)
                    .args(["-h", "127.0.0.1", "-p", &runtime_ports::postgres().to_string(), "-U", user, "-d", "postgres", "-tAc"])
                    .arg("SELECT 1"),
            );
            if code == 0 {
                sql_ready = true;
    ...
    provision_pg_roles(deadline, &pg_bin, log)?;
    ...
    let (code, out, err) = run_capture(deadline,
        Command::new(&psql)
            .args(["-h", "127.0.0.1", "-p", &runtime_ports::postgres().to_string(), "-U", PG_ADMIN_USER, "-d", "postgres", "-tAc"])
            .arg(format!("SELECT 1 FROM pg_database WHERE datname='{PG_DB}'")),
    );
    append_log(log, format!("db exists probe code={code} {out}{err}"));
    if !out.contains('1') {
        let (c2, o2, e2) = run_capture(deadline,
            Command::new(&createdb)
                .args([
                    "-h", "127.0.0.1",
                    "-p", &runtime_ports::postgres().to_string(),
                    "-U", PG_ADMIN_USER,
                    "-O", PG_USER,
                    PG_DB,
                ]),
    ...
    let _ = run_capture(deadline,
        Command::new(&psql)
            .args(["-h", "127.0.0.1", "-p", &runtime_ports::postgres().to_string(), "-U", PG_ADMIN_USER, "-d", PG_DB, "-c"])
    ...
    let (_, verify, _) = run_capture(deadline,
        Command::new(&psql)
            .args(["-h", "127.0.0.1", "-p", &runtime_ports::postgres().to_string(), "-U", PG_ADMIN_USER, "-d", "postgres", "-tAc"])
            .arg(format!("SELECT 1 FROM pg_database WHERE datname='{PG_DB}'")),
    );
...
fn provision_pg_roles(deadline: &Deadline, pg_bin: &Path, log: &mut String) -> Result<(), String> {
    let psql = bin(pg_bin, "psql");
    ...
    let (code, out, _) = run_capture(deadline,
        Command::new(&psql)
            .args(["-h", "127.0.0.1", "-p", &runtime_ports::postgres().to_string(), "-U", session_user, "-d", "postgres", "-tAc"])
...
pub(crate) fn apply_runtime_grants(deadline: &Deadline, log: &mut String) -> Result<(), String> {
...
fn ensure_redis(deadline: &Deadline, redis_bin: &Path, data_dir: &Path, pid_file: &Path, log_file: &Path, log: &mut String) -> Result<(), String> {
    deadline.check()?;
    ...
    if port_open("127.0.0.1", runtime_ports::redis()) {
    ...
    cmd.arg("--port")
        .arg(runtime_ports::redis().to_string())
        .arg("--bind")
        .arg("127.0.0.1")
        .arg("--dir")
        .arg(data_dir)
    ...
    #[cfg(windows)]
    {
        ...
        if let Err(error) = fs::write(pid_file, format!("{pid}\n")) {
            crate::process_deadline::terminate_and_wait(&mut child)?;
            return Err(format!("record redis process: {error}"));
        }
    ...
    #[cfg(not(windows))]
    {
        deadline.wait(&mut child)?;
    }

    if !wait_port(deadline, "127.0.0.1", runtime_ports::redis(), 15, log) {
...
pub fn ensure_native(
    ...
    let startup = Deadline::after(Duration::from_secs(90));
    ...
    if port_open("127.0.0.1", runtime_ports::postgres()) && port_open("127.0.0.1", runtime_ports::redis()) {
        append_log(&mut log, "fast-path: pg+redis already up");
        if let Some(pg_bin) = find_pg_bin() {
            if let Err(e) = provision_pg_roles(deadline, &pg_bin, &mut log) {
                flush_ensure_log(&state_rt, &log);
                return NativeEnsureReport {
                    ok: false,
                    message: e,
    ...
    let pg_was_up = port_open("127.0.0.1", runtime_ports::postgres());
    ...
    let ok = port_open("127.0.0.1", runtime_ports::postgres()) && port_open("127.0.0.1", runtime_ports::redis());
    flush_ensure_log(&state_rt, &log);
    NativeEnsureReport {
        ok,
        message: if ok {
...
pub(crate) fn stop_owned_postgres(data: &Path) -> Result<(), String> {
    ...
    let code = run_status_null(deadline, Command::new(bin(&pg_bin, "pg_ctl"))
...
pub fn stop_native() -> NativeEnsureReport {
    ...
    if postmaster.is_file() {
        ...
        if let Some(pg_bin) = find_pg_bin() {
            if pgdata.join("PG_VERSION").is_file() {
                let code = run_status_null(deadline,
    ...
    if pid_file.is_file() {
    ...
    } else if port_open("127.0.0.1", runtime_ports::redis()) {
        ...
        if let Some(redis_bin) = find_redis_server() {
            if let Some(dir) = redis_bin.parent() {
                ...
                if cli.is_file() {
                    let (c, o, e) = run_capture(deadline,
                        Command::new(&cli)
                            .args(["-p", &runtime_ports::redis().to_string(), "SHUTDOWN", "NOSAVE"]),
                    );

    ...
    <section style={{ display: "grid", gap: "1rem" }}>
      <div style={{ display: "flex", alignItems: "center", gap: "0.75rem" }}>
        <Link href="/admin" style={{ color: "hsl(var(--muted-foreground))" }}>
          {adminMessage(locale, "common.back")}
        </Link>
      </div>
      <AdminPageHeading
        title={adminMessage(locale, "organizationDetail.title")}
        subtitle={adminMessage(locale, "organizationDetail.subtitle")}
      />
      {error ? <ErrorState message={formatAdminError(locale, error)} /> : null}
      {loading ? (
        <LoadingState copy={adminMessage(locale, "organizationDetail.loading")} />
      ) : !organization ? (
        <EmptyState copy={adminMessage(locale, "organizationDetail.notFound")} />
      ) : (
        <>
            ...
            <div style={{ display: "flex", justifyContent: "space-between", gap: "1rem", alignItems: "start" }}>
              <div style={{ display: "grid", gap: "0.35rem" }}>
                ...
                <p style={{ margin: 0, color: "hsl(var(--muted-foreground))" }}>
                  {adminMessage(locale, "common.organizationId")}: {organization.id}
                </p>
              </div>
              <button className="app-button-ghost" disabled={toggleBlockedMutation.isPending} type="button" onClick={() => void handleToggleBlocked()}>
                {toggleBlockedMutation.isPending
                  ...
                  : organization.blocked
                    ? adminMessage(locale, "organizations.unblockOrganization")
                    : adminMessage(locale, "organizations.blockOrganization")}
              </button>
            </div>
            <div style={{ display: "grid", gap: "0.7rem", gridTemplateColumns: "repeat(auto-fit, minmax(12rem, 1fr))" }}>
              <AdminMetricCard label={adminMessage(locale, "common.status")} tone={organization.blocked ? "danger" : "success"} value={orgStatusLabel(locale, organization.blocked)} />
              ...
              <AdminMetricCard
                label={adminMessage(locale, "common.notebooks")}
                value={organization.notebook_count.toString()}
                detail={`${adminMessage(locale, "common.created")} ${formatUnixDate(organization.created_at, locale)}`}
              />
            </div>
          </section>
          ...
          {insightLoading ? (
            <LoadingState copy={adminMessage(locale, "organizationDetail.loadingInsights")} />
          ) : (
            <>
              <div style={{ display: "grid", gap: "0.8rem", gridTemplateColumns: "repeat(auto-fit, minmax(12rem, 1fr))" }}>
                <AdminMetricCard label={adminMessage(locale, "common.period7dRequests")} tone="primary" value={(usage7d?.total_requests ?? 0).toString()} />
                <AdminMetricCard label={adminMessage(locale, "common.period30dRequests")} tone="success" value={(usage30d?.total_requests ?? 0).toString()} />
                <AdminMetricCard label={adminMessage(locale, "common.tokens30d")} tone="warning" value={formatCompactNumber(usage30d?.total_tokens ?? 0)} />
                <AdminMetricCard label={adminMessage(locale, "common.documents30d")} tone="danger" value={(usage30d?.total_documents ?? 0).toString()} />
              </div>
              ...
              <div style={{ display: "grid", gap: "1rem", gridTemplateColumns: "minmax(0, 1fr) minmax(0, 1fr)" }}>
                <section className="app-inline-surface" style={{ display: "grid", gap: "0.8rem" }}>
                  <div className="app-inline-row" style={{ marginBottom: 0 }}>
                    <h3 style={{ margin: 0 }}>{adminMessage(locale, "organizationDetail.teamComposition")}</h3>
                  ...
                  <div style={{ display: "grid", gap: "0.8rem", gridTemplateColumns: "repeat(3, minmax(0, 1fr))" }}>
                    <AdminMetricCard label={adminMessage(locale, "common.owners")} value={ownerCount.toString()} />
                    <AdminMetricCard label={adminMessage(locale, "common.admins")} value={adminCount.toString()} tone="warning" />
                    <AdminMetricCard label={adminMessage(locale, "users.memberRoles")} value={memberCount.toString()} tone="success" />
                  </div>
                  <div style={{ display: "grid", gap: "0.5rem" }}>
                    {recentMembers.map((user) => (
                      <div className="app-inline-row" key={user.id} style={{ marginBottom: 0 }}>
                        ...
                        <span style={{ fontSize: "0.82rem", color: "hsl(var(--muted-foreground))" }}>
                          {user.last_active_at ? formatUnixDate(user.last_active_at, locale) : adminMessage(locale, "common.neverActive")}
                        </span>
                      </div>
                    ))}
                  </div>
                ...
                <section className="app-inline-surface" style={{ display: "grid", gap: "0.8rem" }}>
                  <h3 style={{ margin: 0 }}>{adminMessage(locale, "organizationDetail.operationalEfficiency")}</h3>
                  <div style={{ display: "grid", gap: "0.8rem", gridTemplateColumns: "repeat(2, minmax(0, 1fr))" }}>
                    <AdminMetricCard label={adminMessage(locale, "common.requestsPerUser30d")} value={requestsPerUser30d.toString()} />
                    <AdminMetricCard label={adminMessage(locale, "organizationDetail.notebooksPerUser")} value={notebooksPerUser.toString()} tone="success" />
                  </div>
                  <div className="app-inline-surface" style={{ display: "grid", gap: "0.5rem" }}>
                    <div className="app-inline-row" style={{ marginBottom: 0 }}>
                      <span>{adminMessage(locale, "common.period7dRequests")}</span>
                      <strong>{usage7d?.total_requests ?? 0}</strong>
                    </div>
                    <div className="app-inline-row" style={{ marginBottom: 0 }}>
                      <span>{adminMessage(locale, "common.tokens30d")}</span>
                      <strong>{formatCompactNumber(usage30d?.total_tokens ?? 0)}</strong>
                    </div>
                    <div className="app-inline-row" style={{ marginBottom: 0 }}>
                      <span>{adminMessage(locale, "common.documents30d")}</span>

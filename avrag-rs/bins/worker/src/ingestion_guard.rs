...
pub(crate) fn from_storage_error(error: PgStorageError) -> IngestionError {
    IngestionError::storage_database(error)
}
...
pub(crate) async fn ensure_ingestion_side_effects_allowed(
    ...
    if allowed {
        ...
        Err(IngestionError::storage_lease_lost(format!(
...
pub(crate) async fn verify_uploaded_object_bytes(
    ...
    if let Some(expected_size) = validation.upload_size_bytes {
        ...
        if expected_size != actual_size {
            return Err(IngestionError::upload_tampered(format!(
    ...
    if let Some(expected_sha256) = validation.upload_sha256.as_deref() {
        ...
        if expected_sha256 != actual_sha256 {
            return Err(IngestionError::upload_tampered(

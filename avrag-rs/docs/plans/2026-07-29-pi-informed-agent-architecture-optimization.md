...
| 2026-07-29 | **B1 follow-up + Wave C**：`execute_codegen_bridged` / `BridgeCallObs`（rag-core 仅 `deps.rs` + builder 签名）；`product_contract` facade；`worker_contract::RETRIEVAL_ALIAS_START_METADATA`；`HOST_ONLY_TOOL_NAMES` catalog 排斥测试（`conversation_history_load` 为 dual tool） |

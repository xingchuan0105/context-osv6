...
- 设置 `DATABASE_URL`：尝试连接 PostgreSQL 并执行迁移，`/ready` 返回 `m1-m2-postgres`
...
当前代码已经支持 PostgreSQL 启动路径，但本地 smoke 仍默认基于内存模式完成。
若你的 WSL 环境没有接通 Docker Desktop 或本机 PostgreSQL，请先使用内存模式联调前端协议。

...
`../frontend_rust` 仅作为 legacy/dev-only fallback。
...
该脚本启动 PostgreSQL、Redis、Qdrant 和 MinIO；Milvus 需要单独启动。若本机暂时没有 Milvus，请设置 `RETRIEVAL_BACKEND=legacy` 使用 rollback adapter。
...
- `RETRIEVAL_BACKEND`：默认 `milvus`；需要回滚旧检索路径时设为 `legacy`
...
- 设置 `DATABASE_URL` 且 `RETRIEVAL_BACKEND=legacy`：使用 Qdrant/Tantivy/PostgreSQL BM25 rollback adapter
...
当前代码已经支持 PostgreSQL + Milvus 启动路径，但本地 smoke 仍可基于内存模式完成。

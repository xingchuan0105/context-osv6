...
v5 明确区分四类输入的来源和信任等级。**核心原则：不是"不让看"，而是"看了但防着"。**

| 输入类型 | 来源 | 信任等级 | Plan | Evaluate | Answer | 防护措施 |
|---------|------|---------|------|----------|--------|----------|
| **系统提示** | 平台预定义 | 高 | ✅ 允许 | ✅ 允许 | ✅ 允许 | 不可被用户输入覆盖 |
| **用户输入** | 前端/API | 中 | ✅ 允许（guard 后） | ✅ 允许（guard 后） | ✅ 允许（guard 后） | 必须过 prompt injection 检测 |
| **工具 schema** | Registry 定义 | 高 | ✅ 允许（只读） | ✅ 允许（只读） | ❌ 不需要 | 只暴露签名，不暴露实现 |
| **检索/搜索内容** | RAG/Search 工具 | 低 | ❌ 首轮 Plan 不需要 | ✅ 允许（防护后） | ✅ 允许（防护后） | UntrustedInputProcessor 结构化封装 |
| **工具输出** | 外部 API | 低 | ❌ 首轮 Plan 不需要 | ✅ 允许（防护后） | ✅ 允许（防护后） | UntrustedInputProcessor 结构化封装 |

> ✅ 允许 / ❌ 不需要 / ⚠️ 受限

**关键规则：**
- Planner（首轮 Plan）只需要系统提示 + 用户输入 + 工具 schema，不需要检索内容（还没检索）
- Evaluator（Evaluate 阶段）**必须看到**检索/搜索内容才能判断质量——但内容必须经过结构化封装
- Answer 阶段可以看到所有输入，用于生成自然语言回答
- 任何外部内容（检索结果、网页、API 返回）进入 LLM 前必须经过防护处理
...
> 原始检索内容必须经过结构化封装后才能进入任何 LLM 上下文。安全性靠"看了但防着"来保证，不靠"不让看"。
>
> 具体执行：
> 1. 检索结果进入 Evaluate 或 Answer 前必须经过 `UntrustedInputProcessor` 结构化封装
> 2. 审计系统可以读取原始内容用于追溯
> 3. `content_guard` 在运行时检测并清洗已知 prompt injection 模式
> 4. prompt 中明确声明外部数据的信任等级，指导模型正确对待

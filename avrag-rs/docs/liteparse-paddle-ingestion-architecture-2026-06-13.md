...
> **Status:** 已定稿（产品/工程讨论结论，待实现）  
> **日期:** 2026-06-13（v1.1 审查修订）  
> **取代范围:** 以本文为准，覆盖 `docs/ingestion-routing-discussion-2026-06-10.md` 中关于 MinerU、Paddle 慢线、EdgeParse/lopdf 主路径的待定项  
> **关联:** `docs/adr/0002-ingestion-routing-and-retrieval.md`、`docs/visual-pdf-ingest-requirements-2026-06-10.md`  
> **代码锚点（现网，迁移前）:** `crates/ingestion/src/parser/`、`bins/worker/src/pdf/`、`bins/worker/src/pipeline/`
...
**LiteParse 做统一探针与数字 PDF 抽字；示意图走 MM Embedding；表与扫描走 Paddle AI Studio Jobs API（免费额度）；Excel 单独保留 Office 服务；MinerU 与旧 PDF 路由后端删除。**
...
5. **可降级、可回滚**：OCR 失败有明确兜底 ladder；LiteParse 与新旧链均可用 feature flag 切换。  
6. **先 shadow 再切流再删代码**：禁止未过验收指标就删除现网路径。
...
| **EdgeParse / lopdf（退役）** | 数字 PDF 快抽字 | `PdfParser` + `PdfPageBackend::EdgeParse` | 由 **LiteParse** 替代 |
| **VisualRaster 常规路由（退役）** | 图多页整页 raster | `PdfPageBackend::VisualRaster` | 仅 **E 类兜底** |
| **MinerU（退役）** | 外部图片/复杂版面解析 | `ExternalParseKind::MineruImage`、`MineruClient` | **删除** |
| **Office JVM（部分保留）** | 结构化 Office 解析 | `OfficeParserServiceClient` | **仅 xls/xlsx**；doc/ppt 改 LiteParse |
...
| **doc / docx / ppt / pptx** | LiteParse 转 PDF → 同 PDF 流程 | 不再走 Office JVM；见 §15 转 PDF 失败边界 |
| **xls / xlsx** | **Office 服务（保留）** | 结构化 Sheet/单元格；**禁止** LiteParse+LibreOffice 转 PDF |
| **png / jpg / …** | LiteParse（ImageMagick→PDF 或直接解析）+ Paddle Job + MM | 替代 MinerU 图片路径 |
...
探针实现：**LiteParse 快扫优先**；过渡期可复用现网 `ParseProbe` 信号名，便于 shadow 对比。

### 5.4 流程图

```mermaid
flowchart TB
    IN[PDF / Office→PDF / 图片→PDF]
    IN --> PROBE[LiteParse 快扫探针<br/>ocr_enabled=false]
...
| `liteparse_enabled` | `true` / `false` |
| `paddle_jobs_count` | 本文档消耗的 Job 数 |
| `pdf_route_mode` | `liteparse_hybrid`（替代现网 `hybrid`） |
| `ocr_backend` | 固定 `paddle_jobs`（当存在 OCR 页时） |
...
| 组件 | 阶段 | 动作 |
|------|------|------|
| MinerU | P4 删除代码前 | 路由切断 → shadow 验证 → 删代码与 `MINERU_*` |
| Paddle 慢线路由 | P4 | 删 `PdfPageBackend::PaddleOcr` 分流；**保留** Jobs 客户端 |
| EdgeParse / lopdf | P3 灰度后 | LiteParse 替代；lopdf 仅保留 PDF 切片等工具用途（若仍需要） |
| Tesseract | P1 起 | LiteParse `default-features = false`；从不启用 |
| Office JVM（doc/ppt） | P4 | 删路由；**保留** xlsx 路径 |
| VisualRaster 常规路由 | P2 起 | 仅 E 类 |
...
| 变量 | 说明 | 建议初值 |
|------|------|----------|
| `LITEPARSE_ENABLED` | 启用 LiteParse 主解析 | `0` |
| `LITEPARSE_SHADOW_MODE` | 新链只写对比日志/对象存储，不入索引 | `1`（P2.5 开启） |
| `LITEPARSE_OCR_ENABLED` | LiteParse 内置 OCR | `0` |
...
| 变量 | 说明 |
|------|------|
| `OFFICE_PARSER_BASE_URL` | 仅 `xls` / `xlsx` |
...
## 14. 执行编排：Shadow → 灰度 → 删代码

### 14.1 Shadow 模式（P2.5）

- `LITEPARSE_SHADOW_MODE=1`：现网链 **正式入库**；LiteParse 链并行跑，结果写入 `{object_path}/.shadow/liteparse/{parse_run_id}.json`。  
- 对比维度：抽字率、页型分布、Job 数、块数、bbox 覆盖率。  
- **不**写入 Milvus，**不**影响用户检索。

### 14.2 灰度切换（P3）

- `LITEPARSE_ENABLED=1` + `LITEPARSE_SHADOW_MODE=0`：按 notebook / 租户 / 比例切流。  
- 保留 `LITEPARSE_ENABLED=0` 一键回退至 lopdf + 现网路由（过渡期）。  
- 灰度期间 **双写对比** 可选（仅 staging）。
...
| **LibreOffice** | doc/ppt/xlsx→PDF 转换 | Worker 镜像 | **仅** doc/ppt/图片转 PDF；**xlsx 禁止** |
| **ImageMagick** | 图片→PDF | Worker 镜像 | 独立 png 路径 |
| **Paddle AI Studio** | Jobs OCR | 云端 | Token 走密钥管理 |
| **pdf-renderer** | E 类整页截图 | Sidecar 或现有服务 | 非常规路径 |
| **Office JVM 服务** | Excel  only | 现网 | 不变 |

### 15.1 Office → PDF 失败边界

| 场景 | 行为 |
|------|------|
| LibreOffice 转换失败 | 文档 `Failed` + `ParseWarning`；**不** fallback 至 Office JVM（doc/ppt） |
| PPT 动画/备注/隐藏页 | **不保证**保留；仅保证转版 PDF 可搜 |
| doc/docx 复杂样式 | 接受「打印版 PDF」语义，非 WYSIWYG 编辑 |
...
| 阶段 | 内容 | 验收 |
|------|------|------|
| **P0** | LiteParse 库 + `DocumentIr` 适配器（§6）；离线样本 | 抽字率 ≥ lopdf 或阅读顺序更优 |
| **P1** | A/B 类：LiteParse 抽字 + 抠图 MM | 混合 PDF 同页可搜字可搜图 |
| **P2** | C/D 类：`PaddleJobsOcrService` 按页 Job；§9 合并 | 扫描 PDF 关键词可召回 |
| **P2.5** | **Shadow 模式**（§14.1） | 样本集 shadow diff 无未解释严重回归 |
| **P3** | **灰度切流**（§14.2）；Job 缓存/预算（§13） |  staging E2E + 灰度指标达标 |
| **P4** | 删 MinerU、删 Paddle 慢线路由、删 doc/ppt Office；可选 OCR 网关 | 回归 + §18 全通过 |
| **P5** | 监控大盘、额度告警、文档/runbook 更新 | 生产可观测 |
...
| xlsx | Office 结构化不变 |
| docx / pptx | 转 PDF 后走 PDF 流程 |
| 独立 png | Paddle Job + MM，无 MinerU |
...
| Shadow 块数差异 | 无单页块数 ±50% 且无解释 | shadow diff 报告 |
...
| `docs/visual-pdf-ingest-requirements-2026-06-10.md` | B/E + MM 满足 visual 需求 |
...
| Excel | **单独保留 Office 服务** |
| Word/PPT | LiteParse 转 PDF |

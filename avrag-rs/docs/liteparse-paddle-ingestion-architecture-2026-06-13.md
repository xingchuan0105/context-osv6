...
> **日期:** 2026-06-13（v1.2 — M6 文档对齐 P4 后代码）  
...
文档级路由由 `router/mod.rs` 的 `ParseRoute` 决定（`Pdf` / `OfficeService` / `PaddleOcrImage` / `Local`）。PDF 页内路由分三阶段：
...
| `pdf_route_mode` | PDF：`liteparse_hybrid`；独立图片：`paddle_image` |
...
| Tesseract | 从不启用 |
| Office JVM（doc/ppt） | **已退役**；LibreOffice→PDF + LiteParse |
...
- **验收门禁：** `scripts/run-liteparse-staging-e2e.sh`（product E2E，`phase0-mini.pdf`）。
...
| **LibreOffice** | doc/ppt→PDF 转换 | Worker 镜像 | doc/ppt 专用；**xlsx 禁止** |

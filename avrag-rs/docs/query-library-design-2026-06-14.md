...
> 日期：2026-06-14 ｜ 状态：**已定稿 · V1 已落地**（`frontend_next`，`e3e9a67` + `eaccddd`）
...
### 第二轮（深度调研 + 设计复盘后的修订，2026-06-14 · **已采纳并落地**）

1. **挂载 B → A'**：原"不改 history-pane"在 CSS 上不成立（§6.0），改为 history-pane 内部渲染 + flex 布局改造（已按 A' 实现）。

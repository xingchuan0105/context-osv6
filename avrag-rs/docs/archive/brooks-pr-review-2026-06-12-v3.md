...
| 文件 | 变更 | 目的 |
|------|------|------|
| `test_context/config.rs` | `E2eBootstrapConfig` 增加 `org_id`/`user_id`；写入 `AppConfig` 与 worker env | 使 bootstrap 身份与 `x-org-id`/`x-user-id` 一致 |
...
```
HTTP 客户端 ──► x-org-id / x-user-id = unique_test_identity()  (每测试 UUID)
API bootstrap ──► AppConfig::default()  org_id/user_id = 固定默认值   ← 修复前
Worker env    ──► 未显式设置 NEXT_PUBLIC_DEV_ORG_ID               ← 修复前
```
...
Symptom: `apply_worker_env` 通过 `NEXT_PUBLIC_DEV_ORG_ID` / `NEXT_PUBLIC_DEV_USER_ID` 注入 worker，与前端 public env 命名耦合（[`app-core/config.rs`](../crates/app-core/src/config.rs) L301–302 已有此约定）。
...
Remedy: 在 `E2eBootstrapConfig.apply_worker_env` 旁注释「与 `AppConfig::from_env` 共用键，worker 无独立 AVRAG_ORG_ID」；长期可考虑 `AVRAG_DEV_ORG_ID` 别名。

...
**Health Score:** 99/100
**Trend:** 87 → 99 (+12) — v7 S3/S8 核销 test-kit、app-chat dev-dep 环、WorkspaceStore 双路径、文档漂移

生产依赖图继续无环。v7 卫生项（删除 `avrag-test-kit`、移除 `app-chat → app-bootstrap` dev-dep、`WorkspaceStore` 单一出口、README/baseline 对齐）均已落地。剩余 1 分：`pg_*_store` port_impl 单体体量（历史 Suggestion，非本轮回归）。
...
**Knowledge Duplication — `WorkspaceStore` trait 在 `app-core` 与 `app` 各暴露在两条路径**

Symptom: 同一个 `WorkspaceStore` trait 在 `avrag-rs/crates/app-core` 同时存在于：

- `src/ports/workspace_store.rs`（**唯一真定义**）
- `src/ports/workspaces/workspace_store.rs`（一行 `pub use crate::ports::workspace_store::WorkspaceStore;`）

并在 `crates/app` 镜像出第二份相同的“别名包装层”。`app-core/src/adapters/memory.rs` 走第一条路径，`crates/app/src/services/workspaces/service.rs` 与 `crates/app/tests/notebook_service_contract.rs` 走第二条。`ports/workspaces/mod.rs` 整个文件只有 `pub mod workspace_store;` 一行，是空壳目录。
...
Remedy: 选一条路径作为公共出口（建议保留 `ports::workspaces::workspace_store::WorkspaceStore`，与 `app/src/services/workspaces/` 命名空间对齐），把另一条删掉；同步更新 `adapters/memory.rs` 等少数现存 `use` 语句。
...
v7 S3/S8 完成两项结构性 Warning 与文档 Suggestion：`avrag-test-kit` 已删除、`app-chat` dev-dep 环已断、`WorkspaceStore` 只保留 `ports::workspaces::workspace_store` 出口。legal 版本双源短期由 P0-CON-5 守护（架构层 typeshare 化为可选长期项）。依赖图无环，分层守护脚本仍绿。
...
| B7 `WorkspaceStore` 单一出口 | ✅ trait 定义仅在 `ports/workspaces/workspace_store.rs` |
...
#   -> chat/  mod.rs  workspace_store.rs  notebooks/  rate_limit/  (notebooks/ 是 1 行 re-export 空壳)
```
...
| 日期 | 说明 |
|------|------|
| 2026-06-13 v6 | 本轮：v5 → archive；核销 `app-core` Redis、`app-chat` 千行文件、`share` axum 泄漏；新增 `avrag-test-kit` 死成员、`app-chat` 未用 dev-dep、`WorkspaceStore` 双路径、文档漂移 |

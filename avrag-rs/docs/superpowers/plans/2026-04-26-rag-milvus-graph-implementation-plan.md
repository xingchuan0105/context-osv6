...
pub trait RetrievalDataPlane: Send + Sync {
    ...
    async fn delete_document_index(&self, org_id: OrgId, document_id: Uuid) -> Result<()>;
...
- 所有 search output 必须带 `org_id/doc_id/chunk_id/parse_run_id/retrieval_channel/score_breakdown/source_locator`。
...
- 所有 collection 必须有 scalar fields：`org_id`、`workspace_id` nullable、`doc_id`、`chunk_id` nullable、`parse_run_id`、`doc_version`。
- `search_*` 必须生成服务端 ACL filter：至少 `org_id == ...` 且 `doc_id in [...]`。
...
- 调用 `replace_document_index`，由 adapter 负责删除同 `org_id + doc_id` 的旧检索数据后 upsert 当前 `parse_run_id`。

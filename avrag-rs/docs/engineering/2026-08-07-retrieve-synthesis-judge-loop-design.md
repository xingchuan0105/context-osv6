...
| 非目标 | Judge 内检索或代写终答；host 对照 golden；无限回环；每题长 ReAct critic |
...
**检索只找料且出环硬门；合成只写/改答；短 Judge 只裁决+开方（不检索、不写答）；不合格带意见回合成或回检索，最多 3 次，到顶交卷并降级说明。**
...
| **合成** | 写终稿、按 Judge 意见改稿 | 不替代 Judge 裁决；出检索硬门不在这里重复发明 |
| **短 Judge** | 通过 / 不合格；不合格时去向 + 纠正意见 | **不检索、不写/改用户终答** |
...
- **首入**：据检索材料写用户可见答案。
- **再入**（来自 Judge）：据 **回答纠正意见** 改稿；默认 **不再开检索**（补料只能经 Judge 判去检索后重走检索环）。

### 3.2 出口

- 产出一版终稿 → 进入短 Judge。
- 格式壳（纯代码、host 标签泄漏等）：可在进 Judge 前做 **一轮** 格式修（现有 `check_final_answer` / prose-repair 思路），**不占用** §4.3 的 3 次不合格额度（建议）。
...
- **是**：**一拍裁决**（实现上可为单次 LLM + Judge skill；输入题面、终稿、只读证据视图/`claim_notes` 等）。
- 挂 **Judge skill**（文案在 `prompts/`，第三人称观察语气；禁止命令式「必须改成某某」作为唯一政策——意见陈述事实与张力，合成/检索自行决定怎么改）。
...
- 意见应 **可执行**（例如指出竞争数字并存、某槽在证据中未见），避免空泛「再想想」。
...
| 上限 | **3** |
| 第 1～3 次不合格 | 按 `route` + `advice` 进入对应 loop，再合成后再 Judge |
...
- 内部：完整 Judge report 进 telemetry / 评测旁路。
...
## 5. 旁路（跳过或弱化 Judge / 检索硬门）

产品例外，避免无意义烧预算：

| 场景 | 建议 |
|------|------|
| 题卡 calculation / P-calc-ok 类 | 可弱化检索硬门；Judge 极简或跳过 |
| 天气等：约定 **tool 成功即过** | 有 **Ok `weather_query`** 时跳过 short Judge（实现：`weather_tool_ok`）；不对气温陈述做忠实度裁决 |
...
- 检索 / 合成 / Judge **共享** 产品 token·轮次预算；回环次数 **3** 是 Judge 不合格专用上限，不替代全局预算。
...
- 检索模型 **无交卷权**（对适用 mode）；
- **交付权**在短 Judge「通过」或「到顶强制交卷」；
- host 做 **状态机 + 硬结构门 + 信号路由**，语义对错仍由 Judge skill（模型）陈述，host 不读 golden。
...
| 项 | 约定 |
|----|------|
| LLM 面向指令 | 仅 `avrag-rs/prompts/**`（Judge skill、loop 降级说明、纠正意见包装 observation） |
...
- [ ] 每条成功用户答案均经合成稿；经 Judge 通过或到顶降级二者之一。  
- [ ] Judge 实现与 skill **无** 检索工具、**无** 直接产出替换终答的主路径。  
...
| 日期 | 说明 |
|------|------|
| 2026-08-07 | 初稿：三环、短 Judge（仅裁决+意见）、回环 3、到顶交卷降级；与会话定案一致 |
| 2026-08-07 | **V1 实现**：`LoopExitConfig::{forbid_retrieve_direct_answer,short_judge,judge_max_fail_rounds}`；rag/search YAML 开启；`content_dispatch` 散文 → `BreakToSynthesis`（worker_handoff 除外）；`short_judge` 模块 + `prompts/clusters/answer-judge` + loop 观察；`ReActLoop::run` 合成后 Judge 回环（合成/检索） |
| 2026-08-07 | **V1.1**：`AGENTS.md` 三环 stop 表；`weather_query` Ok 旁路 Judge；`follow_up_after_judge_fail` 纯函数 + 单测（fail→合成/检索观察、第 4 次到顶） |
| 2026-08-07 | **V1.2 review 修复 1–5**：回检索共享 prior token + 最多 2 轮；Judge 前不 stream/Done；仅 **weather-only** Ok 旁路；证据含 code_execution/claim_notes；parse 失败打点 + soft Pass；Judge LLM 失败 fail-open 交卷 |
| 2026-08-07 | **V1.3 二轮 review 5 项**：产品 token 含 synth+judge；`effective_max_judge_fails` / 耗尽强制 ceiling；`deliver_synthesized` 去重；证据分桶截断；`judge-empty-advice.md`；rereretrieve Activity 分计 |
| 2026-08-07 | **V1.4 验收阻断修复**：`mode_assemble` 继承 `forbid_retrieve_direct_answer`/`short_judge`/`judge_max_fail_rounds`；回检索共享 product rounds；回合成注入 draft；`short_judge_report` telemetry；Judge cancel；assemble 断言 |

...
| 🟢 Share port 公开读/邀请仅 token round-trip | ✅ FIXED — `share/tests/share_behavior.rs` 6 测覆盖 `load_shared_workspace` payload 映射、`resolve_public_share_chat_context`、`owner_can_invite_member` + `non_owner_invite_is_rejected_before_store`；MemoryShareStore 现在跟踪 invite 列表 |

...
Phase 1a 法律页与注册同意链路的验收规格。自动化检查见仓库根目录 `scripts/verify-legal-p0.sh`（32 项 P0 + §9.11 签字 1 项 + 7 项 P1 = **40** 项自动化检查）。

## 9.3 P0 验收标准（摘要）

> 项数对照：IA-1..7（7）+ CNT-1..6（6）+ FE-1..6（6）+ UX-1..3（3）+ CON-1..6（6）+ PIPE-1..4（4）= 32 项 P0；§9.11 签字作为第 33 项附加检查；P1-API/FE/PIPE（7）作为第 34–40 项增量检查。
...
- 支付 / 重新确认等场景使用 `POST /api/auth/legal-acceptance`（需登录，`context`: `payment` | `re_acceptance`）

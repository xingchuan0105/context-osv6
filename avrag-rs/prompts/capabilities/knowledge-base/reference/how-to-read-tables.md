...
问：某阶段有多少个活动？
回传：
  grep total_hits=92（该阶段表管道行命中）
  部分活动名在多行出现（多角色/多职责展开）
  另可观察：按「活动」列去重后的项数与 92 不同（若统计出现在回传中）

误读 A：只答去重后约 70，不提 92、不标口径
误读 B：只答 92，并裁定「去重一定错误 / 不许提去重」
观察侧常见完整写法：
  - 按表行（角色展开条目）：92（= total_hits / 行 COUNT）
  - 按活动名称去重：另一整数，并写明去重键
  两种都是回传可支撑的计数方式；工作是并列揭露差异，不是只交付一个未说明口径的整数。
```

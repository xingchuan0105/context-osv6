...
**情境：** 问「某阶段有多少个活动？」  
**回传：** grep `total_hits=92`（该阶段管道行）；活动名在多行出现（多角色展开）；另可观察到按「活动」列去重后的项数与 92 不同（若回传中有该统计）。

| 误读 | 与回传一致的读法 |
|------|------------------|
| 只答去重后约 70，不提 92、不标口径 | 行口径 92（= total_hits）与去重口径并列，并写明统计键 |
| 只答 92 且裁定「去重一定错」 | 两种都是可观察计数；工作是并列揭露差异 |

通则见 **how-to-read-tables**「行数 vs 去重」。

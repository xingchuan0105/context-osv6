...
**Runtime 已注入**：当前 query + 最近 **2** 条 prior user 原文（`[prior_user_query]`）+ Pre-Loop 的 `resolved_query`（检索用，messages 仍见原话）。

**按需调取**（本簇）：
1. 更早或跨 session 历史 → `conversation_history_load`（`query` + 默认 `scope=notebook`；近序 + 中文分词 FTS 混合检索）
2. 长期画像/偏好 → `user_profile_load`（跨会话偏好、专业领域、表达风格时）

1. 检索默认使用 `resolved_query` 作为 `conversation_history_load.query`；回答面向用户原始措辞
2. 消解后仍歧义 → 澄清，不臆造实体

...
- **handoff**：分析散文（或 `internal_worker_handoff_v1` JSON：`summary`、`coverage`（full / partial / insufficient）、`gaps`、`premise_mismatch`）+ 一行 `SELECTED: #n`（worker 实际用到的证据别名，系统按此水合全文）。写 brief 时含 `[premise/归属核对]` 要求（见 orchestrator-base）。

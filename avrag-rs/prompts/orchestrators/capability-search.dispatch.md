...
- **handoff**：`summary`、`key_facts`（来源指针；`basis`=observed|inferred）、`coverage`、`gaps`、`premise_mismatch`（前提/归属与证据不符时 worker 的否决信号）。写 brief 时含 `[premise/归属核对]` 要求（见 orchestrator-base）。

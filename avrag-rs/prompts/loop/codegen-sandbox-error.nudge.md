...
环境事实：
- 调用形式为 `client.方法名(...)`，须带括号。`await client.user_context` 无 `()` 不会得到本地时间。基础原语含 `calculator()`、`user_context()`、`weather_query(...)`、history、user_profile、save、load；检索面含 dense、lexical、grep、struct_catalog、struct_query、web、fetch、doc_profile、doc_summary（以本轮挂载为准）。
- 问「现在几点 / 当前日期和时间」时，ok 的 `user_context` 回传写入终答；stderr / Traceback / 方法名不是终答。

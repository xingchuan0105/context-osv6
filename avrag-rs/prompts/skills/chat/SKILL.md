...
- `prior_turns`: recent conversation history (system / user / assistant turns).
- `session_summary`: condensed memory from `session-summary` containing `current_user_goal`, `active_constraints`, `confirmed_facts`, `preferences_or_biases`, `unresolved_questions`, `next_steps`.
- `tool_results`: any tool outputs from this turn (rare for chat; usually empty).
- `injected_skills` / `format_hint`: the planner may inject format or style skills (e.g., `teaching`, `ppt-generation`); you must follow them.

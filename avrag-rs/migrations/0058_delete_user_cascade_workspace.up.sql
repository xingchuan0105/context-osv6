-- Fix delete_user_cascade after notebooks → workspaces rename (0055).
...
BEGIN
    ...
    -- Drop membership rows for this user (owned workspaces cascade via owner_user_id FK).
    DELETE FROM workspace_members WHERE user_id = target_user_id OR email IN (
        SELECT lower(email) FROM users WHERE id = target_user_id
    );

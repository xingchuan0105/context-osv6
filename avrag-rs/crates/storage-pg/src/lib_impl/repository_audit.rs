...
impl AuditRepository {
    pub async fn append_audit_record(&self, record: &AuditRecord) -> Result<(), PgStorageError> {
        let owner_user_id = Uuid::parse_str(&record.owner_user_id)
            .map_err(|_| PgStorageError::NotFound("invalid audit org id".to_string()))?;
        let context = AuthContext::new(UserId::from(owner_user_id), contracts::auth_runtime::SubjectKind::System);
        let mut tx = self.pool.begin(&context).await?;
        ensure_org_and_actor(tx.inner(), &context).await?;
        sqlx::query(
            r#"
            insert into audit_log (owner_user_id, actor_id, action, resource_type, resource_id, payload, created_at)

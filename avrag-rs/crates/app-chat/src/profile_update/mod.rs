//! Profile inference and persistence, consolidated in one module.
//!
//! `maybe_update_profiles` is the single pipeline entry: it lists the session's
//! messages once, then runs the dream-v2 semantic-delta writer under a single
//! 24h cadence gate.
...
pub(crate) use dream::{LlmProfileDeltaStrategy, ProfileDeltaStrategy};
...
/// Number of most-recent turns fed to the dream layer.
...
/// Build a plain-text rendering of the most recent turns for the dream layer.
...
impl ChatContext {
    /// Persist profile observations for a completed execution.
    ///

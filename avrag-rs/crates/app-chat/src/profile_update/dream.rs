...
use super::merge::{
    apply_profile_delta, parse_profile_delta_response, user_profile_from_value, user_profile_to_value,
};
use super::types::ProfileDelta;
use crate::context::ChatContext;

/// How a semantic delta is obtained. The production strategy asks the LLM;
/// unit tests inject a fake to exercise the gate and merge deterministically.
#[async_trait::async_trait]
pub(crate) trait ProfileDeltaStrategy: Send + Sync {
...
/// Production strategy: LLM-proposed semantic delta.
pub(crate) struct LlmProfileDeltaStrategy;

#[async_trait::async_trait]
impl ProfileDeltaStrategy for LlmProfileDeltaStrategy {
    async fn infer_delta(
        &self,
        ctx: &ChatContext,
        recent_turns: &str,
        ...
        ctx.infer_profile_delta(recent_turns, existing_profile).await
    }
}
...
impl ChatContext {
    ...
    pub(crate) async fn maybe_update_structured_profile(
        ...
        recent_turns: &str,
        strategy: &dyn ProfileDeltaStrategy,
    ) -> bool {
        ...
        let delta = strategy
            .infer_delta(self, recent_turns, &existing_structured_value)
            .await;

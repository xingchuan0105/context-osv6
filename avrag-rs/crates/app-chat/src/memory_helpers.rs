...
pub fn build_citations(context_document: Option<&RetrievedContext>) -> Vec<Citation> {
...
pub fn build_sources(context_document: Option<&RetrievedContext>) -> Vec<SourceRef> {
    let Some(document) = context_document else {
        return Vec::new();
    };
    vec![SourceRef {
...
pub fn agent_name(agent_type: &str, language: Option<&str>) -> &'static str {
    crate::i18n::agent_name(agent_type, language)
}

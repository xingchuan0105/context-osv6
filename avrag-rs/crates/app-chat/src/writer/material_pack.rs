...
use crate::writer::invoker::ResearchOutcome;
...
impl MaterialPack {
    ...
    /// Build the initial pack from the orchestrator's research outcome.
    pub fn from_research(outcome: &ResearchOutcome, workspace_text: &str) -> Self {
...
mod tests {
    ...
    fn empty_outcome(cards: Vec<MaterialCard>, reservoir: Vec<&str>) -> ResearchOutcome {
        ResearchOutcome {
            cards,
    ...
    fn from_research_buckets_rag_and_web() {
        ...
        let outcome = empty_outcome(vec![rag, web], vec!["Rust"]);
    ...
    fn render_appendix_has_sections() {
        ...
        let outcome = empty_outcome(vec![rag], vec!["Rust"]);
        let pack = MaterialPack::from_research(&outcome, "Rust is great");
    ...
    fn merge_respects_limit() {
        let outcome = empty_outcome(vec![], vec![]);

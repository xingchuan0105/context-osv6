...
use common::{
    BackendTrace, Coverage, ExecutePlanRequest, ExecutePlanResponse, RetrievedChunk,
    RetrievalBundle,
};

use crate::retrieval::ScoredChunk;

use super::planner::{build_item_trace, rag_summary_mode};
...
impl RagRuntime {
    pub async fn execute_plan(
        ...
        let item_trace = build_item_trace(&compat_request, &compat_plan);

        let (text_dense_lists, mut degrade_trace) = self
            .retrieve_text_dense_stage(&compat_request, auth, &compat_plan)
            .await?;
        let (bm25_lists, mut stage_degrade) = self
            .retrieve_bm25_stage(&compat_request, auth, &compat_plan)
            .await?;
        degrade_trace.append(&mut stage_degrade);

        let (multimodal_pool, mut stage_degrade) = self
            .retrieve_multimodal_dense_stage(&compat_request, auth, &compat_plan)
            .await?;
        degrade_trace.append(&mut stage_degrade);

        let text_pool = self.merge_text_stage(text_dense_lists, bm25_lists);
        let (reranked_chunks, mut stage_degrade) = self
            .multimodal_rerank_stage(&compat_request.query, text_pool, multimodal_pool)
            .await?;
        degrade_trace.append(&mut stage_degrade);

        let retrieved_chunks = self.cut_final_candidates_stage(reranked_chunks);
        let raw_summary_chunks = self
            .apply_summary_policy(&compat_request, auth, &compat_plan, &retrieved_chunks)
            .await?;
        let answer_context = self.build_answer_context_chunks(&raw_summary_chunks, &retrieved_chunks);
        ...
        Ok(ExecutePlanResponse {
            ...
            coverage: Coverage {
                ...
                summary_chunk_count: answer_context
                    .iter()
                    .filter(|chunk| chunk.chunk_type == "summary")
                    .count(),
            },

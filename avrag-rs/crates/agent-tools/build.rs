...
/// Scanned: prompts/clusters/, prompts/capabilities/*/SKILL.md, prompts/system/.
...
fn main() {
    ...
    // Capability directories (contract.md + SKILL.md + reference/): SKILL.md
    // registers under its frontmatter `name` (stable across moves, e.g. `search`).
    // Shared clusters (memory/docscope/writing/format/heavytail-*/index/...):
    // one SKILL.md per directory.
    let scan_roots = [prompts_root.join("clusters"), prompts_root.join("capabilities")];
    ...
    // Flat system docs (agent-base.md) — capability contracts are NOT scanned
    // as skills (they are assembly-only, mounted via system_prompt_parts).
    for flat_root in ["system"] {
    ...
    generated.push_str("// ADR-0007: clusters + capabilities + system\n\n");
    generated.push_str("{\n");

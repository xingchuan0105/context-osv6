...
use crate::eval::framework::{EvalScore, Evaluator};
use crate::runtime::AgentRunResult;
use common::AppError;
...
impl Evaluator for RedTeamEvaluator {
    async fn evaluate(
        &self,
        case: &crate::eval::framework::EvalCase,
...
fn evaluate_blocked(result: &AgentRunResult) -> (f64, String) {
    ...
    if let Some(crate::runtime::FinalDecision::Clarified { .. }) = result.final_decision {
        return (
            1.0,
...
pub async fn run_redteam_evaluation(
    ...
    agent: &dyn crate::runtime::Agent,
    sink: &dyn crate::events::AgentEventSink,
) -> Result<crate::eval::framework::EvalRun, AppError> {
    use crate::eval::framework::{EvalDatasetSpec, EvalTrigger, EvalTriggerConfig};
    ...
    for case in &dataset.cases {
        ...
        cases.push(crate::eval::framework::EvalCase {
    ...
    let (run, _result) = crate::eval::framework::run_eval_with_trigger(
        &dataset.name,
        "RedTeam",
        "v1",
        cases,

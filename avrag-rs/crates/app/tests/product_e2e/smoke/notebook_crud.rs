...
async fn notebook_crud_lifecycle_via_http() {
    ...
    let notebook_id = created.id.clone();

    let get_resp = ctx.get_notebook(&notebook_id).await.unwrap();
    ...
    let notebooks = list_resp.body_json["notebooks"]
        .as_array()
        .expect("notebooks array");
    assert!(
        notebooks
            .iter()
            .any(|nb| nb.get("id").and_then(|v| v.as_str()) == Some(notebook_id.as_str())),
        "created notebook should appear in list"
    );

    let update_resp = ctx
        .update_notebook(&notebook_id, "crud-renamed", "updated description")
        .await
        .unwrap();
    ...
    let delete_resp = ctx.delete_notebook(&notebook_id).await.unwrap();
    assert_eq!(delete_resp.status, 200, "delete notebook: {delete_resp:?}");

    let gone_resp = ctx.get_notebook(&notebook_id).await.unwrap();
    assert!(

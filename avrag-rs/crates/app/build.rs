...
///
/// All file contents (SKILL.md, assets/, reference/) are embedded as string
...
fn main() {
    ...
    let skills_dir = skills_dir
        .canonicalize()
        .expect("prompts/skills/ must exist");

    let mut generated = String::new();
    generated.push_str("// AUTOGENERATED BY build.rs — do not edit manually.\n");
    generated
        .push_str("// Scans prompts/skills/ at compile time and generates registration calls.\n\n");
    ...
    for entry in fs::read_dir(&skills_dir).expect("read skills dir") {
        ...
        let output_schema = if ref_dir.exists() {
            ...
            if output_schema_path.exists() {
                let schema_md =
                    fs::read_to_string(&output_schema_path).expect("read output-schema.md");
                extract_json_code_block(&schema_md)
            } else {
                None
            }

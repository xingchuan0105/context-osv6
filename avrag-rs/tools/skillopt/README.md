...
> **当前状态：训练已启动（2026-08-01）。** 静态自检（`--check`）通过；评分点修正
> （JUDGE_ERROR 跳过 + non-RAG soft 单维）已提交。

## Seed 对照基线（验收口径）

- 训练前产品全量 149 双轮 PASS：**122/149（81.9%）与 123/149（82.6%）**
  （`v2_20260801-082506` / `v2_20260801-094826`，**旧评测标准**——R1/R2 假失败修正之前）。
- 新评测标准（R1/R2 修正后）下同量级应更高；训练前后对比以**同标准、同 split**为准：
  引擎在训练开始自动对 seed skill 在 val 集跑 baseline（`selection_eval_baseline/`），
  训练结束后对比 `best_skill.md` 的 val/test 分数。
- 验收参考：全量 149 PASS 不低于 122/149；表格题（q088 等）不倒退。
